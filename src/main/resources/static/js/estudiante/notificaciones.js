document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnMarcarTodas').addEventListener('click', async () => {
        try {
            const r = await fetch('/notificaciones/leidas', { method: 'PUT', credentials: 'include' });
...
async function cargarNotificaciones() {
    ...
    try {
        const r = await fetch(`/notificaciones?page=${paginaActual}&size=20`, { credentials: 'include' });
        if (!r.ok) { const d = await r.json(); throw d.errorMsg || `Error ${r.status}`; }
        ...
        lista.querySelectorAll('.notif-item').forEach(el => {
            el.addEventListener('click', () => {
                ...
                handleClick(ids, el.dataset.leida === 'true', el.dataset.url, '/notificaciones');
            });
        });
    } catch (e) {
        lista.innerHTML = '';
...
function iconClass(tipo) {
    return { SOLICITUD_APROBADA: 'success', SOLICITUD_RECHAZADA: 'danger', NUEVO_MENSAJE: 'primary' }[tipo] ?? 'primary';
}

function iconoTipo(tipo) {
    return { SOLICITUD_APROBADA: 'bi-check-circle-fill', SOLICITUD_RECHAZADA: 'bi-x-circle-fill', NUEVO_MENSAJE: 'bi-chat-dots-fill' }[tipo] ?? 'bi-bell-fill';
}
